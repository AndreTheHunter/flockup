...
class EventDetails extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final List<Widget> details = []
      ..add(imageOrPlaceholder(context, event))
      ..addAll(venueSection(context, event))
      ..addAll(overviewSection(context, event));

    return new Scaffold(
        appBar: AppBar(title: new Text(name)),
        body: new Column(children: <Widget>[
          new Expanded(
              child: Padding(
                  padding: const EdgeInsets.all(16.0),
                  child: ListView(children: nonNullWidgets(details))))
        ]));
  }
}
...
List<Widget> venueSection(context, event) {
  ...
  if (isNotNull(venue)) {
    ...
    return [
      new Text('VENUE', style: theme.body2),
      spacer,
...
List<Widget> overviewSection(context, event) {
  ...
  if (isNotNull(details)) {
    return [
      new Text('OVERVIEW', style: theme.body2),
